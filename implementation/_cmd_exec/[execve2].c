...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/08/18 21:32:18 by tidminta          #+#    #+#             */
/*   Updated: 2021/08/18 21:32:19 by tidminta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*_place_holder_get_path(t_msh *msh, t_cut_cmd *cmd)
{
	...
	if (tmp2)
	{
		...
		tmp[0] = tmp2;
		return (get_path(cmd, tmp, -1));
	}	
	else
		return (get_path(cmd, msh->path, -1));
}
...
void	delete_heredoc(t_msh *msh)
{
	...
	if (pid2 == 0)
	{
		...
		to_delete_path = get_path(to_delete, list_to_split(msh->env->head), -1);
