...
/*   Updated: 2020/04/08 15:47:57 by tidminta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_w_p(t_infos_ *st_, t_params_ *p, size_t len, size_t len2)
{
	...
	{
		res = (st_->width > 1) ? st_->width - 2 : 0;
		len2 = (st_->flag_diez) ? res : st_->width;
	...
	{
		res = (st_->arg_hexa) ? len + 2 : len;
		len2 = (st_->flag_diez) ? res : len;
